...
URL: https://share.streamlit.io/dduong19/final-project/main/finalproject.py

Description:
This program answers two queries. The first query is about which borough has the most injuries for pedestrians, bikers,
motorists. The results will be shown in a bar chart. The second query is which borough has the most collisions at what time
intervals. The results will be displayed on a interactive map.
'''
...
# creating a list of the collision counts in each borough
    ...
    if len(boroughCollisions)==4:
        boroughCollisions.append(0)
...
# creating a list of the boroughs, lon, lats, collision counts
    ...
    layer1 = pdk.Layer("ColumnLayer", data=dfmap, get_position='[lon, lat]', auto_Highlight = True, get_elevation= "collisions",
                        get_radius=10000, get_color=[200,200,200], pickable=True, elevationScale = 250, coverage=5,
                       diskResolution=8, wireframe=True)
